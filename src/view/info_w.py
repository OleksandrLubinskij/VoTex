...
class InfoFrame(BaseView):
    def __init__(self, controller, master, **kwargs):
        ...
        self.main_container = ctk.CTkFrame(self, fg_color=config.FG_COLOR)
    ...
    def create_info_card(self, title, text):
        card = ctk.CTkFrame(self.main_container, fg_color=config.FG_COLOR, corner_radius=15, 
                            border_width=1, border_color="#e0e0e0")
        card.pack(fill="x", pady=10, padx=10)
        title_lbl = self.create_label(card, text=title, font_size=20, weight="bold")
        title_lbl.configure(text_color=config.SIGNATURE_GREEN) # Твій фірмовий зелений
        title_lbl.pack(anchor="w", padx=20, pady=(15, 5))
        desc_lbl = self.create_label(card, text=text, font_size=16, weight="normal")
        desc_lbl.configure(wraplength=650, justify="left", text_color=config.TEXT_COLOR)
