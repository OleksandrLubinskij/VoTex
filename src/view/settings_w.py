...
class SettingsFrame(BaseView):
    def __init__(self, master, controller, **kwargs):
        ...
        self.general_section = ctk.CTkFrame(self.main_container, fg_color=config.TEXT_BOX_COLOR, corner_radius=15, border_width=1, border_color="#e0e0e0")
        self.general_section.pack(fill="x", pady=(0, 20), padx=2)
        self.general_section.grid_columnconfigure(1, weight=1)

        # Шлях
        self.create_label(self.general_section, text="Шлях за умовчанням", font_size=20).grid(row=0, column=0, padx=20, pady=20, sticky="w")
        ...
        ctk.CTkFrame(self.general_section, height=2, fg_color=config.FG_COLOR).grid(row=1, column=0, columnspan=2, sticky="ew", padx=15)

        # Тема
        self.create_label(self.general_section, text="Кольорова тема", font_size=20).grid(row=2, column=0, padx=20, pady=20, sticky="w")
        ...
        self.lang_section = ctk.CTkFrame(self.main_container, fg_color=config.TEXT_BOX_COLOR, corner_radius=15, border_width=1, border_color="#e0e0e0")
        self.lang_section.pack(fill="both", expand=True, pady=(0, 20), padx=2)
        
        self.create_label(self.lang_section, text="Доступні мови для вибору", font_size=20).pack(anchor="w", padx=20, pady=(15, 10))
        ...
        for lang_short, lang_full in self.available_langs.items():
            cb = ctk.CTkCheckBox(self.lang_scroll, text=lang_full, font=("Segoe UI", 18), 
                                 hover_color=config.SIGNATURE_GREEN_HOVER, fg_color=config.FG_COLOR)
    ...
    def save_and_notify(self):
        ...
        self.after(2000, lambda: self.save_btn.configure(fg_color=config.SIGNATURE_GREEN, text="Зберегти зміни"))
